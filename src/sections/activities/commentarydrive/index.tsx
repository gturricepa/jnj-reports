...
export const AcitivitieCommentaryDrive = () => {
  const { filteredData, loading } = useFetchData<ActivitesDataPIFS>(
    "commentarydrive.xlsx"
  ); //mudei o xlsx para Operationg group e sector para franchise e Sector  e Region
  ...
  const OkCountNA = filteredDataByOperatingGroupAndSector
    .filter((item) => item.Status === "Complete")
    .filter((region) => region.Region === "NA").length;

  const OkCountLATAM = filteredDataByOperatingGroupAndSector
    .filter((item) => item.Status === "Complete")
    .filter((region) => region.Region === "LATAM").length;

  const pendingCountLATAM = filteredDataByOperatingGroupAndSector
    .filter((item) => item.Status === "Pending")
    .filter((region) => region.Region === "LATAM").length;

  const pendingCountNA = filteredDataByOperatingGroupAndSector
    .filter((item) => item.Status === "Pending")
    .filter((region) => region.Region === "NA").length;

  const okPercetageNA =
    (OkCountNA / filteredDataByOperatingGroupAndSector.length) * 100;

  const okPercetageLATAM =
    (OkCountLATAM / filteredDataByOperatingGroupAndSector.length) * 100;
  ...
  const totalDriversNA = OkCountNA + pendingCountNA;

  const totalDriversLATAM = OkCountLATAM + pendingCountLATAM;
  ...
  const handleDownload = () => {
    ...
    downloadExcel(
      ...
      columnsToDownload,
      "activitiescommentarydrive.xlsx"
    );
  };
  ...
  return (
    <S.Holder>
      <DescTitle title="Commentary Drive" />
                ...
                <S.Escope>
                  ...
                  {region === "LATAM" ? (
                    <S.MapHolder>
                      ...
                      <Card
                        icon={<TrophyOutlined />}
                        total={+okPercetageLATAM.toFixed(2)}
                        text={"Complete"}
                        helper="%"
                      />
                    </S.MapHolder>
